...
<header>
  <div class="brand">
    ...
    <div class="title-area">
      <h1>Review Khung Ảnh Đầu Đã Xóa Logo (Clean Scene Images)</h1>
      <p>Dự án: Time Travel Vlog — Ancient Babylon (570 BC) • 16:9 • Đã Tẩy Logo & Re-upload</p>
    </div>
  </div>
  <div class="stats-bar">
...
function loadState() {{
  try {{
    return JSON.parse(localStorage.getItem('babylon_review_state_v2') || '{{}}');
  }} catch(e) {{
    return {{}};
  }}
}}
...
function saveState(state) {{
  localStorage.setItem('babylon_review_state_v2', JSON.stringify(state));
  updateStats();
}}
