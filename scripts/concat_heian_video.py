#!/usr/bin/env python3
"""
Concatenate all downloaded scene videos for Heian 1000 into a final video.
"""
import sys
import shutil
import subprocess
...
FINAL_FILE = OUT_DIR / "heian_1000_final.mp4"

ffmpeg_bin = shutil.which("ffmpeg") or "ffmpeg"

def run_concat():
    NORM_DIR.mkdir(parents=True, exist_ok=True)
    video_files = sorted(list(VIDEOS_DIR.glob("scene_*.mp4")))
    if not video_files:
        print("[!] No scene videos found in:", VIDEOS_DIR)
        return False
    ...
    # 3. Concat demuxer
    print(f"[*] Concatenating normalized videos into {FINAL_FILE.name}...")
    concat_cmd = [
        ffmpeg_bin, "-y",
        "-f", "concat",
        "-safe", "0",
